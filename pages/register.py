...
def register_user(username, password):
    try:
        cursor.execute("INSERT INTO users (username, password, balance) VALUES (?, ?, ?)", (username, hash_password(password), 100.0))  # 初期残高を100 Coinに設定
        conn.commit()
        return True
    ...
    if register_button:
        if new_username and new_password:
            if register_user(new_username, new_password):
                ...
                # ダッシュボードに遷移するためにクエリパラメータを更新
                st.experimental_set_query_params(page='dashboard')  # クエリパラメータを更新
                st.markdown("[ダッシュボードに移動する](http://localhost:8501/?page=dashboard)")  # ダッシュボードのURLに変更
